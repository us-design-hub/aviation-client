...
/** Shared display primitives for overview pages (dashboard, lessons, billing).
 *  Tone tokens below stay legible in both themes. */
export const TONES = {
...
export function StatTile({
  label, value, unit, hint, icon: Icon, tone = "neutral", href, onClick, active, progress, progressLabel,
}) {
    ...
    <CardContent className="flex h-full flex-col p-5">
      ...
      {hint && (
        <p className="mt-1.5 flex items-center gap-1 text-xs text-muted-foreground">
          {hint}
          {interactive && <ArrowUpRight className="size-3 opacity-0 transition-opacity group-hover:opacity-100" />}
        </p>
      )}
  ...
  const card = (
    <Card className={cn(
      "group h-full gap-0 py-0 text-left transition-all",
      interactive && "hover:-translate-y-0.5 hover:border-golden/40 hover:shadow-md",
      active && "border-golden shadow-sm",
    )}>
      {body}
    </Card>
  );

  if (href) return <Link href={href} className="block h-full rounded-xl">{card}</Link>;
  if (onClick) {
    return (
      <button
        type="button"
