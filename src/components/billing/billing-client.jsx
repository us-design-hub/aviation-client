...
function StatusBadge({ status }) {
  const colors = { PAID: "border-green-200 bg-green-50 text-green-700", PENDING: "border-amber-200 bg-amber-50 text-amber-700", CANCELED: "border-gray-200 bg-gray-50 text-gray-600", PAYMENT_FAILED: "border-red-200 bg-red-50 text-red-700", REFUNDED: "border-blue-200 bg-blue-50 text-blue-700", VOIDED: "border-gray-300 bg-gray-100 text-gray-700", BLOCKED: "border-red-200 bg-red-50 text-red-700", WARNING: "border-amber-200 bg-amber-50 text-amber-700" };
  return <Badge variant="outline" className={colors[status] || ""}>{String(status || "").replaceAll("_", " ")}</Badge>;
}
...
function CustomerBilling({ catalog, summary, refresh }) {
  ...
  return <Tabs defaultValue="balances" className="space-y-5">
    ...
    <TabsContent value="transactions" className="space-y-8">
      <section><h2 className="text-lg font-semibold">Package Purchases</h2>{(summary?.purchases || []).length === 0 ? <p className="mt-2 text-sm text-muted-foreground">No package purchases yet.</p> : summary.purchases.map((item) => <div key={item.id} className="flex flex-col justify-between gap-3 border-b py-4 sm:flex-row sm:items-center"><div><p className="font-medium">{item.package_name}</p><p className="text-sm text-muted-foreground">{formatDate(item.latest_payment_at || item.created_at)} · {formatMoney(item.amount_cents)}</p>{paymentOutcome(item) === "PAYMENT_FAILED" && item.latest_payment_error && <p className="mt-1 text-xs text-red-600">{item.latest_payment_error}</p>}</div><div className="flex flex-wrap items-center gap-2"><StatusBadge status={paymentOutcome(item)} />{item.gateway === "QUICKBOOKS_PAYMENTS" && item.status !== "PENDING" && <Button size="sm" variant="outline" onClick={() => viewReceipts(item.id)}>Receipts</Button>}{item.status === "PENDING" && catalog.paymentGatewayConfigured && <Button size="sm" onClick={() => setCheckout({ purchase: item })}>{paymentOutcome(item) === "PAYMENT_FAILED" ? "Retry Payment" : "Pay Now"}</Button>}{item.status === "PENDING" && <Button size="sm" variant="outline" onClick={() => cancelPurchase(item.id)}>Cancel</Button>}</div></div>)}</section>
    ...
    <PaymentDialog open={Boolean(checkout)} onOpenChange={(value) => { if (!value) { setCheckout(null); refresh(); } }} purchase={checkout?.purchase} selection={checkout?.selection} catalog={catalog} onSuccess={refresh} />
...
function AdminBilling({ overview, catalog, refresh }) {
  ...
  return <div className="space-y-5">{!catalog?.paymentGatewayConfigured && <Alert variant="destructive"><AlertCircle className="h-4 w-4" /><AlertDescription>Online checkout is disabled. Missing backend configuration: {(catalog?.paymentConfigurationMissing || []).join(", ") || "unknown"}.</AlertDescription></Alert>}<div className="flex items-center justify-between border-b pb-4"><div><p className="font-medium">Custom hour purchases</p><p className="text-sm text-muted-foreground">Allow students and renters to purchase quantities outside the published blocks.</p></div><Switch checked={Boolean(catalog?.customHoursEnabled)} onCheckedChange={updateCustomHours} /></div><Tabs defaultValue="payables" className="space-y-5"><TabsList className="h-auto flex-wrap justify-start"><TabsTrigger value="payables">Instructor Payables</TabsTrigger><TabsTrigger value="purchases">Pending Purchases</TabsTrigger><TabsTrigger value="payments">Online Payments</TabsTrigger><TabsTrigger value="debt">Student Debt</TabsTrigger><TabsTrigger value="packages">Packages</TabsTrigger></TabsList>
    ...
    <TabsContent value="payments">{onlinePurchases.length === 0 ? <p className="text-sm text-muted-foreground">No completed, canceled, or failed online payments yet.</p> : onlinePurchases.map((item) => <div key={item.id} className="flex flex-col justify-between gap-3 border-b py-4 sm:flex-row sm:items-center"><div><p className="font-medium">{item.user_name} · {item.package_name}</p><p className="text-sm text-muted-foreground">{formatDate(item.latest_payment_at || item.paid_at || item.created_at)} · {formatMoney(item.amount_cents)}</p>{item.gateway_reference && <p className="mt-1 font-mono text-xs text-muted-foreground">{item.gateway_reference}</p>}{paymentOutcome(item) === "PAYMENT_FAILED" && item.latest_payment_error && <p className="mt-1 text-xs text-red-600">{item.latest_payment_error}</p>}</div><div className="flex flex-wrap items-center gap-2"><StatusBadge status={paymentOutcome(item)} />{item.status === "PAID" && <><Button size="sm" variant="outline" onClick={() => reversePayment(item.id, "void")} disabled={Boolean(saving)}>Void</Button><Button size="sm" variant="destructive" onClick={() => reversePayment(item.id, "refund")} disabled={Boolean(saving)}>Refund</Button></>}</div></div>)}</TabsContent>
