  ...
  return <Tabs defaultValue="balances" className="space-y-5">
    ...
    <TabsContent value="balances" className="space-y-5">
      ...
      <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
        ...
        {user?.role === "STUDENT" && <SummaryCard label="Outstanding Instructor Debt" value={formatMoney(debt.outstandingAmountCents)} icon={DollarSign} tone={debt.status === "BLOCKED" ? "text-red-600" : "text-foreground"} />}
      </div>
    </TabsContent>
...
function AdminBilling({ overview, catalog, refresh }) {
  ...
  const payInstructor = async (instructorId) => { const form = payments[instructorId] || {}; try { setSaving(instructorId); await billingAPI.recordInstructorPayment(instructorId, { amount: form.amount, note: form.note }); toast.success("Instructor payment recorded"); setPayments((current) => ({ ...current, [instructorId]: { amount: "", note: "" } })); await refresh(); } catch (error) { toast.error(error.response?.data?.message || "Could not record payment"); } finally { setSaving(null); } };
  const recordStudentPayment = async (userId) => { const form = studentPayments[userId] || {}; try { setSaving(`student-${userId}`); await billingAPI.recordStudentInstructionPayment(userId, { hours: form.hours, note: form.note }); toast.success("Student instructor payment recorded"); setStudentPayments((current) => ({ ...current, [userId]: { hours: "", note: "" } })); await refresh(); } catch (error) { toast.error(error.response?.data?.message || "Could not record student payment"); } finally { setSaving(null); } };
  return <div className="space-y-5"><div className="flex items-center justify-between border-b pb-4"><div><p className="font-medium">Custom hour purchases</p><p className="text-sm text-muted-foreground">Allow students and renters to request quantities outside the published blocks.</p></div><Switch checked={Boolean(catalog?.customHoursEnabled)} onCheckedChange={updateCustomHours} /></div><Tabs defaultValue="payables" className="space-y-5"><TabsList><TabsTrigger value="payables">Instructor Payables</TabsTrigger><TabsTrigger value="purchases">Pending Purchases</TabsTrigger><TabsTrigger value="debt">Student Debt</TabsTrigger></TabsList>
    <TabsContent value="payables" className="space-y-5"><div className="grid gap-4 sm:grid-cols-3"><SummaryCard label="Instructor Earnings" value={formatMoney(overview?.payables?.totalEarnedCents)} icon={ReceiptText} /><SummaryCard label="Paid to Instructors" value={formatMoney(overview?.payables?.totalPaidCents)} icon={CheckCircle2} /><SummaryCard label="Currently Owed" value={formatMoney(overview?.payables?.totalOutstandingCents)} icon={DollarSign} /></div>{(overview?.payables?.instructors || []).map((item) => <div key={item.instructorId} className="border-b py-5"><div className="mb-3 flex flex-wrap items-center justify-between gap-2"><div><p className="font-semibold">{item.instructorName}</p><p className="text-sm text-muted-foreground">{item.earnedHours.toFixed(1)} hrs taught · {formatMoney(item.paidAmountCents)} paid</p></div><p className="text-lg font-bold">{formatMoney(item.outstandingAmountCents)} owed</p></div>{item.outstandingAmountCents > 0 && <div className="grid gap-3 md:grid-cols-[160px_1fr_auto]"><Input type="number" min="0.01" step="0.01" placeholder="Payment amount" value={payments[item.instructorId]?.amount || ""} onChange={(event) => setPayments((current) => ({ ...current, [item.instructorId]: { ...current[item.instructorId], amount: event.target.value } }))} /><Textarea rows={1} placeholder="Payment period or reference" value={payments[item.instructorId]?.note || ""} onChange={(event) => setPayments((current) => ({ ...current, [item.instructorId]: { ...current[item.instructorId], note: event.target.value } }))} /><Button onClick={() => payInstructor(item.instructorId)} disabled={!payments[item.instructorId]?.amount || saving === item.instructorId}>Record Payment</Button></div>}</div>)}</TabsContent>
    ...
    <TabsContent value="debt">{debtStudents.length === 0 ? <p className="text-sm text-muted-foreground">No students currently have instructor debt.</p> : debtStudents.map((item) => <div key={item.id} className="border-b py-5"><div className="mb-3 flex flex-wrap items-center justify-between gap-2"><div><p className="font-medium">{item.name}</p><p className="text-sm text-muted-foreground">{item.email}</p></div><div className="text-right"><div className="flex items-center gap-2"><StatusBadge status={item.instructionBilling.status} /><span className="font-semibold">{item.instructionBilling.outstandingHours.toFixed(1)} hrs</span></div><p className="mt-1 text-sm text-muted-foreground">{formatMoney(item.instructionBilling.outstandingAmountCents)}</p></div></div><div className="grid gap-3 md:grid-cols-[160px_1fr_auto]"><Input type="number" min="0.1" max={item.instructionBilling.outstandingHours} step="0.1" placeholder="Hours paid" value={studentPayments[item.id]?.hours || ""} onChange={(event) => setStudentPayments((current) => ({ ...current, [item.id]: { ...current[item.id], hours: event.target.value } }))} /><Textarea rows={1} placeholder="Payment reference or note" value={studentPayments[item.id]?.note || ""} onChange={(event) => setStudentPayments((current) => ({ ...current, [item.id]: { ...current[item.id], note: event.target.value } }))} /><Button onClick={() => recordStudentPayment(item.id)} disabled={!studentPayments[item.id]?.hours || saving === `student-${item.id}`}>{saving === `student-${item.id}` ? "Recording..." : "Record Payment"}</Button></div></div>)}</TabsContent>
  </Tabs></div>;
}
