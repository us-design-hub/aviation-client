...
function CustomerBilling({ catalog, summary, refresh }) {
  ...
  const startPurchase = async (packageId, hours) => {
    if (!catalog.paymentGatewayConfigured) return requestPurchase(packageId, hours);
    try {
      setSubmitting(packageId);
      const response = await billingAPI.createPurchase({ packageId, customHours: hours });
